...
function getOffsetRange(daysBefore) {
  ...
  const startDate = DateTime.now().toUTC().minus({ days: daysBefore }).toFormat("yyyy-LL-dd");
  return `${startDate}..*`;
}
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
...
async function getWorkflowId(workflowFilename) {
  try {
    ...
    const workflowsIdsMap = Object.fromEntries(
  ...
  Workflows: [${workflowsWithIds}]`
    ...
    if (error3 instanceof Error) {
      core3.error(
        `getWorkflowId: An unexpected error has occurred: ${error3.message}`
      );
...
async function getWorkflowRuns(workflowId, tryUseBranch = false) {
  ...
  Runs Fetched: [${runs.map(
        (run2) => `${run2.id} (Attempt ${run2.attempt})`
      )}]`
    );
    return runs;
  } catch (error3) {
    if (error3 instanceof Error) {
      core3.error(
        `getWorkflowRuns: An unexpected error has occurred: ${error3.message}`
      );
      error3.stack && core3.debug(error3.stack);
    }
    throw error3;
...
async function getCheckId(checkSuiteId, checkName) {
  try {
    ...
    const checkIdsMap = Object.fromEntries(
  ...
  Checks: [${checksWithIds}]`
    ...
    if (error3 instanceof Error) {
      core3.error(
        `getCheckId: An unexpected error has occurred: ${error3.message}`
      );
      error3.stack && core3.debug(error3.stack);
    }
    throw error3;
...
async function getRunState(runId, runType) {
  ...
  Conclusion: ${response.data.conclusion}`
    ...
    if (error3 instanceof Error) {
      core3.error(
        `getRunState: An unexpected error has occurred: ${error3.message}`
      );
      error3.stack && core3.debug(error3.stack);
    }
    throw error3;
...
async function run() {
  try {
    ...
    core4.info(
      `Awaiting completion of local Workflow Run ${config2.workflow}...
