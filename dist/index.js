  ...
  "lib/action.js"(exports2) {
    ...
    function getConfig() {
      return {
        ...
        workflow: core2.getInput("workflow", { required: true }),
        checkName: (() => {
          const input = core2.getInput("check_name");
          return input === "" ? void 0 : input;
        })(),
        timeoutMins: getNumberFromValue(core2.getInput("timeout_mins")) || WORKFLOW_TIMEOUT_MINUTES,
        pollIntervalMs: getNumberFromValue(core2.getInput("poll_interval_ms")) || POLL_INTERVAL_MS
      };
    }
...
var require_utils2 = __commonJS({
  "node_modules/@actions/github/lib/internal/utils.js"(exports2) {
    ...
    var httpClient = __importStar2(require_http_client());
...
var require_utils5 = __commonJS({
  "lib/utils.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.getElapsedTime = exports2.sleep = exports2.getOffsetRange = exports2.getBranchName = void 0;
    var core2 = __importStar2(require_core());
    ...
    exports2.getBranchName = getBranchName;
    function getOffsetRange(daysBefore) {
      if (daysBefore < 1) {
        throw new Error(`daysBefore must be greater than 1, received: ${daysBefore}`);
      }
      const startDate = luxon_1.DateTime.now().toUTC().minus({ days: daysBefore }).toFormat("yyyy-LL-dd");
      return `${startDate}..*`;
    }
    exports2.getOffsetRange = getOffsetRange;
    function sleep(ms) {
      return new Promise((resolve) => setTimeout(resolve, ms));
    }
...
var require_api = __commonJS({
  "lib/api.js"(exports2) {
    ...
    var octokit;
    function init(cfg) {
      config = cfg || (0, action_12.getConfig)();
    ...
    async function getWorkflowId(workflowFilename) {
      try {
        ...
        const workflowsIdsMap = Object.fromEntries(response.data.workflows.map((workflow) => [
        ...
        core2.debug(`Fetched Workflows:
  Repository: ${github.context.repo.owner}/${github.context.repo.repo}
  Total Workflows: ${response.data.total_count}
  Workflows: [${workflowsWithIds}]`);
        const id = workflowsIdsMap[workflowFilename];
        if (id === void 0) {
          throw new Error(`Failed to get Workflow ID for '${workflowFilename}', available workflows: [${workflowsWithIds}]`);
        }
        return id;
      } catch (error) {
        if (error instanceof Error) {
          core2.error(`getWorkflowId: An unexpected error has occurred: ${error.message}`);
          error.stack && core2.debug(error.stack);
        }
        throw error;
      }
    }
    exports2.getWorkflowId = getWorkflowId;
    ...
    async function getWorkflowRunId(workflowId) {
      ...
      core2.debug(`Workflow Run ID Found:
  ...
  Run Status: ${workflowRun.status || "null"}`);
    ...
    (function(WorkflowRunStatus2) {
      ...
      WorkflowRunStatus2["InProgress"] = "in_progress";
      WorkflowRunStatus2["Completed"] = "completed";
    })(WorkflowRunStatus = exports2.WorkflowRunStatus || (exports2.WorkflowRunStatus = {}));
    async function getWorkflowRuns(workflowId, tryUseBranch = false) {
      try {
        const branchName = tryUseBranch ? (0, utils_12.getBranchName)(github.context.ref) : void 0;
        const response = await octokit.rest.actions.listWorkflowRuns(__spreadValues({
          ...
          created: (0, utils_12.getOffsetRange)(1),
          per_page: 50
        }));
        if (response.status !== 200) {
        ...
        const runs = response.data.workflow_runs.filter((workflowRun) => workflowRun.head_sha === github.context.sha).map((workflowRun) => ({
          ...
          status: workflowRun.status || void 0
        }));
        if (runs.length > 1) {
          runs.sort((a, b) => {
        ...
        core2.debug(`Fetched Workflow Runs:
  Repository: ${github.context.repo.owner}/${github.context.repo.repo}
  Workflow ID: ${workflowId}
  ...
  Runs Fetched: [${runs.map((run2) => `${run2.id} (Attempt ${run2.attempt})`)}]`);
        ...
        if (error instanceof Error) {
          core2.error(`getWorkflowRunIds: An unexpected error has occurred: ${error.message}`);
          error.stack && core2.debug(error.stack);
        }
        throw error;
      }
    }
    exports2.getWorkflowRuns = getWorkflowRuns;
    var WorkflowRunConclusion;
    (function(WorkflowRunConclusion2) {
      WorkflowRunConclusion2["Success"] = "success";
      WorkflowRunConclusion2["Failure"] = "failure";
      WorkflowRunConclusion2["Neutral"] = "neutral";
      WorkflowRunConclusion2["Cancelled"] = "cancelled";
      WorkflowRunConclusion2["Skipped"] = "skipped";
      WorkflowRunConclusion2["TimedOut"] = "timed_out";
      WorkflowRunConclusion2["ActionRequired"] = "action_required";
    })(WorkflowRunConclusion = exports2.WorkflowRunConclusion || (exports2.WorkflowRunConclusion = {}));
  ...
  Conclusion: ${response.data.conclusion}`);
        ...
        if (error instanceof Error) {
          core2.error(`getWorkflowRunState: An unexpected error has occurred: ${error.message}`);
          error.stack && core2.debug(error.stack);
        }
        throw error;
      }
    }
    exports2.getWorkflowRunState = getWorkflowRunState;
      ...
      if (status === WorkflowRunStatus.Completed) {
        switch (conclusion) {
          ...
          case WorkflowRunConclusion.TimedOut:
            core2.error(`Run has failed with conclusion: ${conclusion}`);
            core2.setFailed(conclusion);
            break;
          default:
            core2.setFailed(`Unknown conclusion: ${conclusion}`);
            break;
        }
        return {
          completed: true,
          conclusion: conclusion || void 0
        };
      }
      return {
        completed: false
    ...
    exports2.getWorkflowRunStatus = getWorkflowRunStatus;
  }
});
...
var INITIAL_WAIT_MS = 10 * 1e3;
async function run() {
  try {
    const config = (0, action_1.getConfig)();
    const startTime = Date.now();
    (0, api_1.init)(config);
    const timeoutMs = config.timeoutMins * 60 * 1e3;
    let attemptNo = 0;
    let elapsedTime = Date.now() - startTime;
    core.info(`Awaiting completion of local Workflow Run ${config.workflow}...
  Workflow: ${config.workflow}
` + (config.checkName ? `  Check: ${config.checkName}
` : "") + `  Timeout: ${config.timeoutMins} (mins)`);
    await (0, utils_1.sleep)(INITIAL_WAIT_MS);
    const workflowId = await (0, api_1.getWorkflowId)(config.workflow);
    let workflowRunId;
    while (elapsedTime < timeoutMs) {
      attemptNo++;
      elapsedTime = Date.now() - startTime;
      if (workflowRunId === void 0) {
        workflowRunId = await (0, api_1.getWorkflowRunId)(workflowId);
      }
      if (workflowRunId !== void 0) {
